...
Webull E-Documents: https://www.webull.com/edocs/
"""
...
class WebullBrokerage(BaseBrokerage):
    ...
    async def _is_logged_in(self) -> bool:
        try:
            return await self.page.locator(
                ".user-center, [data-testid='user-avatar'], .account-info"
    ...
    async def _get_accounts(self) -> list[AccountInfo]:
        ...
        pattern = r"((?:Individual|Roth IRA|Traditional IRA|Cash|Margin)[^*·•]*?)[\s]*[*·•]+(\d{4})"
    ...
    async def _navigate_to_statements(self, account: AccountInfo) -> None:
        ...
        # Filter for statements type
        try:
            stmt_tab = self.page.locator("text=/[Ss]tatement/i").first
            if await stmt_tab.is_visible(timeout=2000):
                await stmt_tab.click()
                await self.page.wait_for_timeout(2000)
        except Exception:
            pass
