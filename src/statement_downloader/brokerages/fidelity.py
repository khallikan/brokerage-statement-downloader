...
Fidelity statements URL: https://digital.fidelity.com/ftgw/digital/portfolio/statements
"""
...
class FidelityBrokerage(BaseBrokerage):
    ...
    NOTE: Selectors are best-effort and will need verification against the
    live Fidelity site.
    """
    ...
    async def _get_accounts(self) -> list[AccountInfo]:
        ...
        # Fidelity shows accounts like "Individual - Brokerage ****1234"
        # or "ROTH IRA ****5678"
        pattern = r"((?:Individual|Roth IRA|Traditional IRA|Rollover IRA|SEP IRA|401\(k\)|Brokerage|Cash Management)[^*]*)\*+(\d{4})"
    ...
    async def _navigate_to_statements(self, account: AccountInfo) -> None:
        ...
        # Select the account if there's an account picker
        try:
            ...
            if await picker.is_visible(timeout=2000):
                await picker.click()
    ...
    async def _get_available_statements(self, account: AccountInfo) -> list[StatementInfo]:
        ...
        for row in rows:
            ...
            if date:
                link = row.locator(
                    "a[href*='.pdf'], a[href*='download'], button"
                ).first
